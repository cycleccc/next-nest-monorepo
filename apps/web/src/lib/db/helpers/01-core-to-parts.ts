...
function sanitizeParts<T extends { type: string; [k: string]: any }>(
    ...
    return parts.filter(
        (part) =>
            !(part.type === 'reasoning' && part.reasoning === 'undefined'),
    );
}
...
async function migrateMessages() {
    ...
    for (let i = 0; i < chats.length; i += BATCH_SIZE) {
                ...
                try {
                    ...
                    for (const msg of projectedUISection) {
                        ...
                        if (msg.role === 'assistant') {
                            const voteByMessage = votes.find(
                                (v) => v.messageId === msg.id,
                            );
                            if (voteByMessage) {
        ...
        for (
            let j = 0;
            j < newMessagesToInsert.length;
            j += INSERT_BATCH_SIZE
        ) {
            const messageBatch = newMessagesToInsert.slice(
                j,
                j + INSERT_BATCH_SIZE,
            );
